...
session = requests_cache.CachedSession("stock_etl.cache")
session.headers["User-agent"] = "educative-project/1.0"
...
if __name__ == "__main__":
    ...
    if args.stock_collect_mode == "normal":
        yesterday = (
            datetime.strptime(args.execution_date, "%Y-%m-%d") - timedelta(days=1)
        ).strftime("%Y-%m-%d")
